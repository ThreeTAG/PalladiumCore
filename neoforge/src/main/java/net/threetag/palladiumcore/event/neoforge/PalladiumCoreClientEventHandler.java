...
@EventBusSubscriber(modid = PalladiumCore.MOD_ID, bus = EventBusSubscriber.Bus.GAME, value = Dist.CLIENT)
public class PalladiumCoreClientEventHandler {
    ...
    public static void screenInitPost(InputEvent.MouseScrollingEvent e) {
        if (InputEvents.MOUSE_SCROLLING.invoker().mouseScrolling(Minecraft.getInstance(), e.getScrollDeltaX(), e.getScrollDeltaY(), e.isLeftDown(), e.isMiddleDown(), e.isRightDown(), e.getMouseX(), e.getMouseY()).cancelsEvent()) {
            e.setCanceled(true);
        }
    }
    ...
    @SubscribeEvent(priority = EventPriority.HIGH)
    public static void clientTickPre(ClientTickEvent.Pre e) {
        ClientTickEvents.CLIENT_PRE.invoker().clientTick(Minecraft.getInstance());
    }

    @SubscribeEvent(priority = EventPriority.HIGH)
    public static void clientTickPost(ClientTickEvent.Post e) {
